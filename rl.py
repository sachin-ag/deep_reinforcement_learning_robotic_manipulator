...
class DDPG(object):
	def __init__(self, a_dim, s_dim, a_bound, mem_cap = 10000):
		self.memory_capacity = mem_cap
		self.memory = np.zeros(
			(self.memory_capacity, s_dim * 2 + a_dim + 1), dtype=np.float32)
		self.pointer = 0
		self.memory_full = False
		self.sess = tf.Session(config=tf.ConfigProto(
			intra_op_parallelism_threads=44, inter_op_parallelism_threads=44))
	...
	def _build_a(self, s, scope, trainable):
		with tf.variable_scope(scope):
			net = tf.layers.dense(
				s, 500, activation=tf.nn.relu, name='l1', trainable=trainable)
			net2 = tf.layers.dense(net, 1000, activation=tf.nn.relu, name='l2', trainable=trainable)
			a = tf.layers.dense(net2, 500, activation=tf.nn.relu,
								name='a', trainable=trainable)
			b = tf.layers.dense(
				a, self.a_dim, activation=tf.nn.tanh, name='b', trainable=trainable)
	...
	def _build_c(self, s, a, scope, trainable):
		with tf.variable_scope(scope):
			...
			net2 = tf.layers.dense(net, 1000, activation=tf.nn.relu, name='c1', trainable=trainable)
			c = tf.layers.dense(net2, 500, activation=tf.nn.tanh,
								name='c2', trainable=trainable)
			return tf.layers.dense(c, 1, trainable=trainable)  # Q(s,a)

	def save(self):
		saver = tf.train.Saver()
		saver.save(self.sess, './checkpoint/params', write_meta_graph=False)

	def restore(self):
		saver = tf.train.Saver()
		saver.restore(self.sess, './checkpoint/params')
