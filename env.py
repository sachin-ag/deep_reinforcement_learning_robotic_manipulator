...
class ClutteredPushGrasp:
    ...
    def set_random_goal(self):
        x = np.random.uniform(-0.224, 0.224)
        y = np.random.uniform(-0.224, 0.224)
        z = np.random.rand()
        self.goal = [x, y, z]
    ...
    def update_reward_ddpg(self):
        ...
        if (self.goal[0] - 0.05 < pos[0] < self.goal[0] + 0.05) and (
                ...
                self.goal[2] - 0.2 < pos[2] < self.goal[2] + 0.2):
            r += 1
            self.on_goal += 1
            if self.on_goal >= 20:
                done = True
    ...
    def generate_state(self, done=False):
        ...
        for joint_pos in joints_pos:
            s.extend([joint_pos[0]/0.224, joint_pos[1]/0.224, joint_pos[2], (joint_pos[0] -
                     goal[0])/0.224, (joint_pos[1] - goal[1])/0.224, (joint_pos[2] - goal[2])])
        s.extend([ee_pos[0]/0.224, ee_pos[1]/0.224, ee_pos[2]/1, (ee_pos[0] -
                 goal[0])/0.224, (ee_pos[1] - goal[1])/0.224, (ee_pos[2] - goal[2])])
        if done:
            s.append(1)
        else:
            s.append(0)
